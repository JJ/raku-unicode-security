...
my %confusables = from-json
"resource/{$dist.meta<resources>[0]}".IO.slurp; my
@confusables-sources; my @confusables-targets;

for %confusables.kv -> $key, @values { for @values -> $v { push
    @confusables-sources: $key; push @confusables-targets: $v; } }

my %confusables-ws = from-json
"resource/{$dist.meta<resources>[1]}".IO.slurp; my
%confusables-ws-sets;

for %confusables-ws.keys -> $key { for %confusables-ws{$key}.keys ->
    $k { %confusables-ws-sets{$key}{$k} = set
    %confusables-ws{$key}{$k}.list; } }

sub confusables( $c where %confusables{$c} ) is export { return
    %confusables{$c} }
...
sub confusable( $this-string, $that-string ) is export { return
    skeleton( $this-string) eq skeleton( $that-string ); }

sub soss( $string ) is export is pure { my %soss; for $string.comb ->
    $c { my $script = $c.uniprop("Script") // "Unknown";
    %soss{$script}.push: $c if $script eq none("Common","Inherited");
    } return %soss; }

sub whole-script-confusable( $target, $str ) is export { my
    $norm-target = $target.wordcase; my %soss = soss($str.NFD.Str) ||
    return False; my @scripts = %soss.keys; return False if
    @scripts.elems > 1; my $source = @scripts.pop; my $char-set =
    %confusables-ws-sets{$source}{$norm-target}; return True if
    $char-set ∩ %soss{$source}.list.Set;

}

sub mixed-script-confusable( $str ) is export { my %soss =
    soss($str.NFD.Str); say %soss.keys; for %soss.keys -> $source { my
    $sum = 0; for %soss.keys -> $target { next if $target eq $source;
    my $char-set = %confusables-ws-sets{$target}{$source}; last unless
    $char-set ∩ %soss{$target}.list.Set; $sum++; } return True if 1 ==
    ( %soss.keys.elems - $sum ); } return False;
    
}

sub mixed_script ($str) is export { return 1 < soss $str; }
